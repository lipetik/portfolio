...
E_teoretical = []
for i in range(len(n)):
    E_teoretical.append(((i) + 0.5)*w)

############## Plot ##############################
# plt.plot(n, eig_values, label="Variational method", marker='o')
# plt.plot(n, E_teoretical, label="Theoretical", marker='o')
...
# data = column_stack([n, eig_values, E_teoretical])
# label = ['n', 'Basis', 'Teórico']
# label_row = ','.join(label)
# header = label_row
# savetxt('oscilator_df.csv', data, delimiter=',', header=header)
